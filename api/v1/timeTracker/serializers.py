...
class TimeLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TimeLog
        fields = ['id', 'project', 'start_at', 'finish_at',
                  'duration', 'status', 'url', 'user']

    def validate(self, attrs):
        if attrs['duration'] is None and attrs['start_at'] is None:
            raise serializers.ValidationError(
                "Duration and Start At cannot be null at the same time")
        if attrs['start_at'] is not None and \
           attrs['finish_at'] is not None and \
           attrs['start_at'] > attrs['finish_at']:
            raise serializers.ValidationError(
                "Finish At cannot be larger than start_at")
        return attrs
